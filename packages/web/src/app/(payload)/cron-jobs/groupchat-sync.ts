...
export const setupGroupchatSync = async (payload: Payload) => {
  console.info('Setting up groupchats!');
  await ensureTypesenseCollectionExists();

  const everyHour = '0 * * * *';
  schedule(everyHour, () => syncGroupchatsToTypesense(payload), {
    runOnInit: true,
  });
};
...
const syncGroupchatsToTypesense = async (payload: Payload) => {
